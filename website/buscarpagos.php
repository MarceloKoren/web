...
<?php
             conectar();
             $consulta = "SELECT COUNT(*) as totalregistros FROM `movimientos` WHERE ( 
                                                                                 codigodebarras LIKE '%$busqueda%' OR
                                                                                 referencia LIKE '%$busqueda%' OR 
                                                                                 cuenta LIKE '%$busqueda%' OR
                                                                                 mediodepago LIKE '%$busqueda%' )  "; 
             ...
             echo "Nro de registros :". $totalregistros;
             ...
             {   
              ...
              $consulta = "SELECT * FROM movimientos  WHERE ( 
                                                              codigodebarras LIKE '%$busqueda%' OR
                                                              referencia LIKE '%$busqueda%' OR 
                                                              cuenta LIKE '%$busqueda%' OR
                                                              mediodepago LIKE '%$busqueda%' )  LIMIT $desde,$porpagina" ; 
...
<nav>
          ...
          <?php
            ...
            if($pagina > 1){
              echo "<a class='page-link' href='?pagina=1&busqueda=".$busqueda."'>Primera</a>";
              echo "<a class='page-link' href='?pagina=".($pagina - 1)."&busqueda=".$busqueda."'><<</a>";
            }
            for($i=$startloop;$i <= $endloop;$i++){
              ...
              if($pagina < $endloop){
                echo "<a class='page-link' href='?pagina=".($pagina + 1)."&busqueda=".$busqueda."'>>></a>";
                echo "<a class='page-link' href='?pagina=".$totalpaginas."&busqueda=".$busqueda."'>Última</a>";
              }
