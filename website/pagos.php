...
<br>
<h1 class="h2">Pagos por Cuenta digital</h1>
<nav class="navbar navbar-light bg-light">
    <form action="importar.php" method="post" enctype="multipart/form-data" id="import_form" class="form-inline mt-2 mt-md-0">
      <div>
        <input type="file" name="file" />
      </div>
      <div class="form-group">
         <label for="exampleFormControlSelect1">-Seleccionar Cuenta :</label>
           <select class="form-control" id="cuentadigital" name="cuentadigital">
               <option>....</option>
               <option>Caja Oficina</option>
               <option>MP y Transferencias</option>
               <option>korenasociados</option>
               <option>riaaravena</option>
               <option>rialaurita</option>
               <option>riaronnie</option>
               <option>korenfernanda</option>
           </select>
  </div>
      <div>
...
<?php
             conectar();
             $consulta = "SELECT COUNT(*) as totalregistros FROM `movimientos` "; 
             ...
             {   
              ...
              $consulta = "SELECT * FROM movimientos ORDER BY id_movimiento ASC LIMIT $desde,$porpagina"; 
...
<tbody>
            <?php    foreach ($pagos as $pago):   ?>
                <tr>
                  <td><?php echo utf8_encode($pago[3])   ?></td>
